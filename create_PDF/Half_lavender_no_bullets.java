...
public class Half_lavender_no_bullets {
	static void pdfGenerate(float w,float h,Document document) throws IOException //changed PdfDocument to Document
	{
		...
		final String background="/Users/charu/Desktop/resume_pdf/lavender.png";
		...
		final String pic="/Users/charu/Desktop/resume_pdf/pp3.jpg";
		...
		Style normal1 = new Style();
		PdfFont font = PdfFontFactory.createFont(FontConstants.TIMES_ROMAN);
		normal1.setFont(font).setFontSize(14).setBold().setFontColor(Color.WHITE);
		
		Style normal2 = new Style();
		normal2.setFont(font).setFontSize(14).setBold();
		
		Style para_text= new Style();
		para_text.setFontColor(Color.WHITE);
		
		while ((line = br.readLine()) != null) {
			
			if(count==1)
			{
				Text t=new Text(line).addStyle(normal1);
				p1=new Paragraph().add(t);
				count=2;
			}
			else
			{
				Text t2=new Text(line).addStyle(para_text);
				p2=new Paragraph().add(t2);
				count=1;
			}
...
//			document.add(p);
		...
		if(!work.equals("NIL"))
		{
			Text t=new Text(line).addStyle(normal2);
